defmodule Mithril.TokenAPI.Deactivator do
  @moduledoc false
  ...
  defp state_options do
    ...
    limit = Confex.get_env(:mithril_api, __MODULE__)[:limit]
  ...
  def do_deactivate_old_password_tokens(%{limit: limit, expiration_days: expiration_days}) do
    subquery_ids =
      ...
      |> where([t, u], u.password_set_at <= datetime_add(^NaiveDateTime.utc_now(), ^(-1 * expiration_days), "day"))
      |> limit(^limit)
