defmodule Core.Authentication.APITest do
  ...
  alias Core.Authentication.{Factor, Factors}
  ...
  describe "verify otp" do
    test "case insensitive OTP key" do
      ...
      assert {:ok, _, :verified} =
               Authentication.verify_otp(
                 "+380901112233",
                 %Token{id: "email@example.com"},
                 otp_code
               )
    end
  end
  ...
  describe "OTP requests limit" do
    ...
    test "NOT timed out but reached max send attempts" do
      ...
      max_logins_period = Confex.fetch_env!(:core, :"2fa")[:otp_send_counter_max]
