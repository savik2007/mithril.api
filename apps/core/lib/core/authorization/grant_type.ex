defmodule Core.Authorization.GrantType do
  @moduledoc false
  ...
  def validate_client_allowed_grant_types(nil, _grant_type),
    do: Error.invalid_client("Invalid client id.")
  ...
  def validate_client_allowed_scope(
  ...
  def validate_user_allowed_scope(
        %User{roles: user_roles, global_roles: global_roles},
        requested_scope
      ) do
    case requested_scope_allowed?(
           join_user_role_scopes(user_roles ++ global_roles),
           requested_scope
         ) do
      true ->
        :ok
      ...
      false ->
        Error.invalid_request("User requested scope that is not allowed by role based access policies.")
    end
  end
  ...
  def requested_scope_allowed?(allowed_scope, requested_scope) do
    ...
    Core.Utils.List.subset?(allowed_scope, requested_scope)
  end

  @doc """
  Fetch scope by itself for trusted Client and empty scope param
  """
  def prepare_scope_by_client(
        %Client{id: id},
        %User{roles: user_roles, global_roles: global_roles},
        nil
      ) do
    trusted_client_ids = Confex.fetch_env!(:core, :trusted_clients)
