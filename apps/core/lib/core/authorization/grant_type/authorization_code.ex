defmodule Core.Authorization.GrantType.AuthorizationCode do
  @moduledoc false
  ...
  alias Core.Error
  alias Core.TokenAPI
  alias Core.Utils.RedirectUriChecker
  ...
  defp validate_client_match(%{details: %{"client_id" => client_id}}, %Client{id: id})
       when id == client_id,
       do: :ok
  ...
  defp validate_redirect_uri(redirect_uri, requested_redirect_uri) do
    case Regex.match?(
           RedirectUriChecker.generate_redirect_uri_regexp(redirect_uri),
