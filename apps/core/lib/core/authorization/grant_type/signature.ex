defmodule Core.Authorization.GrantType.Signature do
  @moduledoc false

  import Core.Authorization.GrantType
  ...
  @mpi_api Application.fetch_env!(:core, :api_resolvers)[:mpi]
  @signature_api Application.fetch_env!(:core, :api_resolvers)[:digital_signature]
    ...
    with %Ecto.Changeset{valid?: true} <- changeset(attrs),
         {:ok, %{"data" => data}} <-
           @signature_api.decode_and_validate(
             attrs["signed_content"],
             attrs["signed_content_encoding"],
             attrs
           ),
         {:ok, %{"content" => content, "signer" => signer}} <-
           process_digital_signature_data(data),
  ...
  defp changeset(attrs) do
    types = %{
      ...
      signed_content_encoding: :string,
      client_id: Ecto.UUID,
      scope: :string
    }
  ...
  defp validate_content_jwt(_),
    do: Error.jwt_invalid("Signed content does not contain field jwt.")

  defp validate_signer_tax_id(%{"drfo" => tax_id})
       when is_binary(tax_id) and byte_size(tax_id) > 0,
       do: {:ok, tax_id}

  defp validate_signer_tax_id(_),
    do: {:error, {:"422", "Digital signature signer does not contain drfo."}}

  def validate_user(%User{is_blocked: false, person_id: id} = user)
  ...
  def validate_user(_),
    do: {:error, {:access_denied, "Person with tax id from digital signature not found."}}
  ...
  defp check_person_status(%{"status" => @person_inactive}),
    do: {:error, {:access_denied, "Person not found."}}

  defp check_person_status(_), do: :ok

  defp validate_person_tax_id(%{"tax_id" => person_tax_id}, tax_id)
       when person_tax_id == tax_id do
    :ok
  end
