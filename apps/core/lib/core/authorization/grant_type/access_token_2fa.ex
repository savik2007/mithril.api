defmodule Core.Authorization.GrantType.AccessToken2FA do
  @moduledoc false
  ...
  def validate_authorization_header(%{"token_value" => token_value}) when is_binary(token_value),
    do: :ok
  ...
  defp check_factor_value(%Factor{factor: factor})
       when is_binary(factor) and byte_size(factor) > 0 do
    :ok
  end
  ...
  defp create_access_token(%Token{details: details} = token) do
    ...
    # creates token with scope that stored in detais.scope_request
    {scope, details} = Map.pop(details, "scope_request", @scope_app_authorize)
  ...
  defp increase_otp_error_counter_or_block_user(%User{priv_settings: priv_settings} = user) do
    otp_error_max = Confex.fetch_env!(:core, :"2fa")[:user_otp_error_max]
