defmodule Core.Authorization.GrantType.Password do
  @moduledoc false
  ...
  alias Core.Authorization.LoginHistory
  alias Core.Clients
  alias Core.ClientTypeAPI.ClientType
  alias Core.Error
  alias Core.ReCAPTCHA
  alias Core.TokenAPI
  alias Core.UserAPI
  ...
  def authorize(attrs) do
    ...
    with %Ecto.Changeset{valid?: true} <- changeset(attrs),
         ...
         {:ok, token} <-
           create_token_by_grant_type(factor, user, client, attrs["scope"], grant_type),
  ...
  defp validate_user_password(%User{id: id, password_set_at: password_set_at}, _grant_type) do
    expiration_seconds = Confex.fetch_env!(:core, :password)[:expiration] * 60 * 60 * 24
  ...
  defp validate_user_by_client(%User{tax_id: tax_id}, %{
  ...
  defp validate_token_scope_by_grant(@grant_type_change_password, _),
    do: Error.invalid_scope(["user:change_password"])
  ...
  defp maybe_send_otp(user, %Factor{} = factor, token),
    do: Authentication.send_otp(user, factor, token)
  ...
  defp map_next_step({:error, :sms_not_sent}),
    do: {:error, {:service_unavailable, "SMS not sent. Try later"}}
