defmodule Core.Authorization.LoginHistory do
  @moduledoc """
  ...
  alias Core.UserAPI.User.LoginHstr
  ...
  def check_failed_login(%User{} = user, @type_password) do
    conf = Confex.fetch_env!(:core, :password)
  ...
  def add_failed_login(%User{} = user, @type_password = type) do
    do_add_login(user, type, Confex.fetch_env!(:core, :password)[:max_failed_logins], false)
  end
  ...
  def add_login(%User{} = user, @type_otp = type, is_success) do
    do_add_login(
      user,
      type,
      Confex.fetch_env!(:core, :"2fa")[:otp_send_counter_max],
      is_success
    )
  end
