defmodule Core.Authorization do
  ...
  defp validate_client_is_blocked(%Client{is_blocked: true}),
    do: Error.access_denied("Client is blocked.")
  ...
  defp validate_redirect_uri(%{connections: connections}, redirect_uri) do
    ...
    Enum.reduce_while(connections, err, fn connection, acc ->
      case Regex.match?(
             RedirectUriChecker.generate_redirect_uri_regexp(connection.redirect_uri),
             redirect_uri
           ) do
        true -> {:halt, :ok}
        false -> {:cont, acc}
