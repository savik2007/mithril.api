defmodule Core.Authorization.Tokens do
  @moduledoc false
  ...
  alias Core.Authorization.BrokerScope
  alias Core.Authorization.GrantType
  alias Core.Authorization.GrantType.AccessToken2FA
  alias Core.Authorization.GrantType.AuthorizationCode
  alias Core.Authorization.GrantType.Password
  alias Core.Authorization.GrantType.RefreshToken
  alias Core.Authorization.GrantType.Signature
  alias Core.Clients
  ...
  def create_by_grant_type(%{"grant_type" => @grant_type_2fa_authorize} = params),
    do: AccessToken2FA.authorize(params)

  def create_by_grant_type(%{"grant_type" => "refresh_2fa_access_token"} = params),
    do: AccessToken2FA.refresh(params)

  def create_by_grant_type(%{"grant_type" => "authorization_code"} = params),
    do: AuthorizationCode.authorize(params)

  def create_by_grant_type(%{"grant_type" => "digital_signature"} = params),
    do: Signature.authorize(params)

  def create_by_grant_type(%{"grant_type" => "refresh_token"} = params),
    do: RefreshToken.authorize(params)
  ...
  def approve_factor(attrs) do
    with :ok <- AccessToken2FA.validate_authorization_header(attrs),
         ...
         %Factor{} = factor <- Factors.get_factor_by!(where_factor),
         :ok <-
           AccessToken2FA.verify_otp(token.details[@factor_field], token, attrs["otp"], user),
  ...
  defp validate_approve_token(_),
    do: Error.access_denied("Invalid token type. Init factor at first")

  defp validate_token_type(%Token{name: @access_token_2fa}, %Factor{factor: v})
       when is_nil(v) or "" == v,
       do: :ok

  defp validate_token_type(%Token{name: @access_token}, %Factor{factor: val})
       when byte_size(val) > 0,
       do: :ok
  ...
  def verify_client_token(token_value, api_key) do
    ...
    with false <- expired?(token),
         ...
         {:ok, token, mis_client_id} <-
           BrokerScope.put_broker_scope_into_token_details(token, client, api_key) do
      {:ok, token, mis_client_id}
    else
      {:error, _} = err -> err
