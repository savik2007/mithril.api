defmodule Core.UserAPI do
  ...
  defp create_or_update_user_factor(
         %{user: %{factor: %Factor{} = factor} = user},
         %{"factor" => _} = attrs
       ) do
    case enabled_2fa?(attrs) do
      true ->
  ...
  defp enabled_2fa?(attrs) do
    case Map.has_key?(attrs, "2fa_enable") do
      ...
      _ -> Confex.fetch_env!(:core, :"2fa")[:user_2fa_enabled?]
    end
  end
  ...
  def merge_user_priv_settings(%User{priv_settings: priv_settings} = user, new_settings)
