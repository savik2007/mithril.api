defmodule Core.Authentication do
  ...
  alias Core.Authentication.OTPSend
  ...
  @type_sms "SMS"
  @sms_api Application.fetch_env!(:core, :api_resolvers)[:sms]
  ...
  defp check_sent_otps(otp_key) do
    conf = Confex.fetch_env!(:core, :"2fa")
  ...
  defp maybe_send_otp(otp, factor) do
    case Confex.fetch_env!(:core, :"2fa")[:sms_enabled?] do
  ...
  def generate_otp_key(prefix, value) when is_binary(prefix) and is_binary(value),
    do: prefix <> "===" <> value
  ...
  def generate_otp_message(code) do
    code_mask = "<otp.code>"
    sms_template = Confex.fetch_env!(:core, :"2fa")[:otp_sms_template]
  ...
  def generate_nonce_for_client(client_id) when is_binary(client_id) do
    ttl = {Confex.fetch_env!(:core, :ttl_login), :minutes}
