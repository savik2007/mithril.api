defmodule Core.OTP do
  ...
  defp initialize_attrs(key) do
    otp_length = Confex.fetch_env!(:core, :"2fa")[:otp_length]
  ...
  defp verify_max_attemps(%OTPSchema{attempts_count: attempts_count}) do
    if attempts_count < Confex.fetch_env!(:core, :"2fa")[:otp_max_attempts],
      do: :ok,
  ...
  defp otp_completed(%OTPSchema{} = otp) do
    ...
    |> update_otp(%{
      ...
      active: false,
      attempts_count: otp.attempts_count + 1
    })
  ...
  """
  @spec update_otp(otp :: OTPSchema.t(), %{}) ::
          {:ok, OTPSchema.t()} | {:error, Ecto.Changeset.t()}
